...
const TutorCourseCard = ({
  ...
  const handleContinue = () => {
    // Navigate to tutor course management page
    const courseSlug = title.toLowerCase().replace(/\s+/g, '-');
    navigate(`/tutor/course/${courseSlug}`);
  };
      ...
      <div className="">
        ...
        <div className="flex items-center justify-between mt-4 mb-2">
          <div className="flex items-center gap-2">
            {studentAvatars.slice(0, 3).map((avatar, index) => (
              <img
                key={index}
                src={avatar}
                alt={`Student ${index + 1}`}
            ...
            {studentCount > 3 && (
              <span className="text-12 font-aileron_r text-brand_secondary ml-1">
                +{studentCount - 3} Students
              </span>
            )}
            {studentCount <= 3 && studentCount > 0 && (
              <span className="text-12 font-aileron_r text-brand_secondary ml-1">
